...
def check_linstor_packages(host):
    if not host.check_packages_available(LINSTOR_PACKAGES):
        raise Exception(
            "Unable to find LINSTOR packages in the yum repositories of {}".format(host)
        )
...
def hosts_with_linstor(host, additional_repos):
    ...
    try:
        wait_for_not(
            lambda: master.ssh_with_result(["linstor", "node", "list"]).returncode
        )
