...
def host_without_zfs(host: Host) -> Generator[Host]:
...
@pytest.fixture(scope='package')
def zpool_vol0(sr_disk_wiped: str, host_with_zfs: Host) -> Generator[None]:
...
def vdi_on_zfs_sr(zfs_sr: SR) -> Generator[VDI]:
    vdi = zfs_sr.create_vdi('ZFS-local-VDI-test')
    yield vdi
    vdi.destroy()

@pytest.fixture(scope='module')
def vm_on_zfs_sr(host: Host, zfs_sr: SR, vm_ref: str) -> Generator[VM]:
