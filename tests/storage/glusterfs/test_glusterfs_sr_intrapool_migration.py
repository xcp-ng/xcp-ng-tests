...
class Test:
    def test_live_intrapool_shared_migration(self, host: Host, hostA2: Host, vm_on_glusterfs_sr: VM,
                                             image_format: ImageFormat) -> None:
        sr = vm_on_glusterfs_sr.get_sr()
        live_storage_migration_then_come_back(vm_on_glusterfs_sr, host, hostA2, sr, image_format)

    def test_cold_intrapool_migration(
            self, host: Host, hostA2: Host, vm_on_glusterfs_sr: VM, nfs_sr: SR, image_format: ImageFormat) -> None:
        cold_migration_then_come_back(vm_on_glusterfs_sr, host, hostA2, nfs_sr, image_format)

    def test_live_intrapool_migration(
            self, host: Host, hostA2: Host, vm_on_glusterfs_sr: VM, nfs_sr: SR, image_format: ImageFormat) -> None:
        live_storage_migration_then_come_back(vm_on_glusterfs_sr, host, hostA2, nfs_sr, image_format)
