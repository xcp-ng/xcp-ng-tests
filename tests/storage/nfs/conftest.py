...
@pytest.fixture
def dispatch_nfs(request: pytest.FixtureRequest) -> Generator[SR | VDI | VM]:
...
@pytest.fixture(scope='package')
def nfs_sr(host: Host, image_format: ImageFormat, nfs_device_config: dict[str, str]) -> Generator[SR]:
...
def vdi_on_nfs_sr(nfs_sr: SR) -> Generator[VDI]:
    vdi = nfs_sr.create_vdi('NFS-VDI-test')
    yield vdi
    vdi.destroy()

@pytest.fixture(scope='module')
def vm_on_nfs_sr(host: Host, nfs_sr: SR, vm_ref: str) -> Generator[VM]:
...
@pytest.fixture(scope='package')
def nfs4_sr(host: Host, image_format: ImageFormat, nfs4_device_config: dict[str, str]) -> Generator[SR]:
...
@pytest.fixture(scope='module')
def vdi_on_nfs4_sr(nfs4_sr: SR) -> Generator[VDI]:
    vdi = nfs4_sr.create_vdi('NFS4-VDI-test')
    yield vdi
    vdi.destroy()

@pytest.fixture(scope='module')
def vm_on_nfs4_sr(host: Host, nfs4_sr: SR, vm_ref: str) -> Generator[VM]:
