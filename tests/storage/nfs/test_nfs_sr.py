...
class TestNFSSRCreateDestroy:
    ...
    def test_create_and_destroy_sr(self, host: Host, dispatch_nfs: dict[str, str]):
...
class TestNFSSR:
    ...
    def test_quicktest(self, dispatch_nfs: SR):
        sr = dispatch_nfs
        sr.run_quicktest()

    @pytest.mark.parametrize('dispatch_nfs', ['vdi_on_nfs_sr', 'vdi_on_nfs4_sr'], indirect=True)
    def test_vdi_is_not_open(self, dispatch_nfs: VDI):
        vdi = dispatch_nfs
    ...
    @pytest.mark.usefixtures('vm_ref')
    @pytest.mark.parametrize('dispatch_nfs', ['vm_on_nfs_sr', 'vm_on_nfs4_sr'], indirect=True)
    def test_start_and_shutdown_VM(self, dispatch_nfs: VM):
        vm = dispatch_nfs
        vm.start()
        vm.wait_for_os_booted()
        vm.shutdown(verify=True)
    ...
    @pytest.mark.usefixtures('vm_ref')
    @pytest.mark.parametrize('dispatch_nfs', ['vm_on_nfs_sr', 'vm_on_nfs4_sr'], indirect=True)
    def test_snapshot(self, dispatch_nfs: VM):
        vm = dispatch_nfs
        vm.start()
        try:
            vm.wait_for_os_booted()
    ...
    @pytest.mark.usefixtures('vm_ref')
    @pytest.mark.parametrize('dispatch_nfs', ['vm_on_nfs_sr', 'vm_on_nfs4_sr'], indirect=True)
    def test_reboot(self, host: Host, dispatch_nfs: VM):
        vm = dispatch_nfs
        sr = vm.get_sr()
