...
class Test:
    ...
    @pytest.mark.parametrize('dispatch_nfs', ['vm_on_nfs_sr', 'vm_on_nfs4_sr'], indirect=True)
    def test_cold_intrapool_migration(self, host: Host, hostA2: Host, dispatch_nfs: VM, local_sr_on_hostA2: SR):
        cold_migration_then_come_back(dispatch_nfs, host, hostA2, local_sr_on_hostA2)

    @pytest.mark.parametrize('dispatch_nfs', ['vm_on_nfs_sr', 'vm_on_nfs4_sr'], indirect=True)
    def test_live_intrapool_migration(self, host: Host, hostA2: Host, dispatch_nfs: VM, local_sr_on_hostA2: SR):
