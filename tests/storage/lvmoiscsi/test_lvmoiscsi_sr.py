...
class TestLVMOISCSISR:
    ...
    def test_quicktest(self, lvmoiscsi_sr: SR):
        lvmoiscsi_sr.run_quicktest()

    def test_vdi_is_not_open(self, vdi_on_lvmoiscsi_sr: VDI):
        assert not vdi_is_open(vdi_on_lvmoiscsi_sr)

    @pytest.mark.small_vm # run with a small VM to test the features
    @pytest.mark.big_vm # and ideally with a big VM to test it scales
    def test_start_and_shutdown_VM(self, vm_on_lvmoiscsi_sr: VM):
    ...
    @pytest.mark.big_vm
    def test_snapshot(self, vm_on_lvmoiscsi_sr: VM):
        vm = vm_on_lvmoiscsi_sr
        vm.start()
        try:
            vm.wait_for_os_booted()
    ...
    @pytest.mark.reboot
    @pytest.mark.small_vm
    def test_reboot(self, host: Host, lvmoiscsi_sr: SR, vm_on_lvmoiscsi_sr: VM):
        sr = lvmoiscsi_sr
        vm = vm_on_lvmoiscsi_sr
