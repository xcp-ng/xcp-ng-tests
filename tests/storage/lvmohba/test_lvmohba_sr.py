...
class TestLVMoHBACBT(CBTTest):
    ...
    def test_snapshot_with_cbt(self, host: Host, lvmohba_sr: SR, vdi_cbt_on_lvmohba_sr: VDI,
                               defer: Defer) -> None:
        self._test_snapshot_with_cbt(host, lvmohba_sr, vdi_cbt_on_lvmohba_sr, defer)

    def test_cbt_on_snapshot_chain(self, host: Host, lvmohba_sr: SR, vdi_cbt_on_lvmohba_sr: VDI,
                                   defer: Defer) -> None:
        self._test_cbt_on_snapshot_chain(host, lvmohba_sr, vdi_cbt_on_lvmohba_sr, defer)

    def test_cbt_parent_disable_does_not_affect_snapshot(self, host: Host, lvmohba_sr: SR,
                                                         vdi_cbt_on_lvmohba_sr: VDI, defer: Defer) -> None:
        self._test_cbt_parent_disable_does_not_affect_snapshot(host, lvmohba_sr, vdi_cbt_on_lvmohba_sr, defer)

    def test_cbt_data_destroy(self, host: Host, lvmohba_sr: SR, vdi_cbt_on_lvmohba_sr: VDI, defer: Defer) -> None:
        self._test_cbt_data_destroy(host, lvmohba_sr, vdi_cbt_on_lvmohba_sr, defer)

    @pytest.mark.small_vm
    def test_changed_blocks_tracking(self, host: Host, lvmohba_sr: SR, vdi_cbt_on_lvmohba_sr: VDI,
                                     vm_on_lvmohba_sr: VM, defer: Defer) -> None:
        self._test_changed_blocks_tracking(host, lvmohba_sr, vdi_cbt_on_lvmohba_sr, vm_on_lvmohba_sr, defer)

    @pytest.mark.small_vm
    def test_incremental_snap_scenario(self, host: Host, lvmohba_sr: SR, vdi_cbt_on_lvmohba_sr: VDI,
                                       vm_on_lvmohba_sr: VM, defer: Defer) -> None:
        self._test_incremental_snap_scenario(host, lvmohba_sr, vdi_cbt_on_lvmohba_sr, vm_on_lvmohba_sr, defer)

    def test_changed_blocks_empty_after_snapshot(self, host: Host, lvmohba_sr: SR,
                                                 vdi_cbt_on_lvmohba_sr: VDI, defer: Defer) -> None:
        self._test_changed_blocks_empty_after_snapshot(host, lvmohba_sr, vdi_cbt_on_lvmohba_sr, defer)

    @pytest.mark.small_vm
    def test_cbt_after_coalesce(self, host: Host, lvmohba_sr: SR, vdi_cbt_on_lvmohba_sr: VDI,
                                vm_on_lvmohba_sr: VM) -> None:
        self._test_cbt_after_coalesce(host, lvmohba_sr, vdi_cbt_on_lvmohba_sr, vm_on_lvmohba_sr)

    def test_cbt_persist_after_pbd_replug(self, host: Host, lvmohba_sr: SR, vdi_cbt_on_lvmohba_sr: VDI,
                                          ) -> None:
        self._test_cbt_persist_after_pbd_replug(host, lvmohba_sr, vdi_cbt_on_lvmohba_sr)
