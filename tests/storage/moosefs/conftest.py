...
def pool_with_moosefs_installed(pool_with_saved_yum_state: Pool) -> Generator[Pool]:
...
@pytest.fixture(scope='package')
def pool_with_moosefs_enabled(pool_with_moosefs_installed: Pool) -> Generator[Pool]:
...
@pytest.fixture(scope='package')
def moosefs_sr(moosefs_device_config: dict[str, str], pool_with_moosefs_enabled: Pool) -> Generator[SR]:
...
def vdi_on_moosefs_sr(moosefs_sr: SR) -> Generator[VDI]:
    vdi = moosefs_sr.create_vdi('MooseFS-VDI-test')
    yield vdi
    vdi.destroy()

@pytest.fixture(scope='module')
def vm_on_moosefs_sr(host: Host, moosefs_sr: SR, vm_ref: str) -> Generator[VM]:
