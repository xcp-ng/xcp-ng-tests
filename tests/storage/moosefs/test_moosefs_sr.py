...
class TestMooseFSSRCreateDestroy:
    ...
    def test_create_moosefs_sr_without_mfsmount(self, host: Host, moosefs_device_config: dict[str, str]):
    ...
    def test_create_and_destroy_sr(self, moosefs_device_config: dict[str, str], pool_with_moosefs_enabled: Pool):
...
class TestMooseFSSR:
    ...
    def test_quicktest(self, moosefs_sr: SR):
        moosefs_sr.run_quicktest()

    def test_vdi_is_not_open(self, vdi_on_moosefs_sr: VDI):
        assert not vdi_is_open(vdi_on_moosefs_sr)

    @pytest.mark.small_vm # run with a small VM to test the features
    @pytest.mark.big_vm # and ideally with a big VM to test it scales
    def test_start_and_shutdown_VM(self, vm_on_moosefs_sr: VM):
        vm = vm_on_moosefs_sr
        vm.start()
    ...
    @pytest.mark.big_vm
    def test_snapshot(self, vm_on_moosefs_sr: VM):
        vm = vm_on_moosefs_sr
        vm.start()
        try:
            vm.wait_for_os_booted()
    ...
    def test_moosefs_missing_client_scan_fails(self, host: Host, moosefs_sr: SR):
        sr = moosefs_sr
    ...
    def test_moosefs_missing_client_pbd_plug_fails(self, host: Host, moosefs_sr: SR):
        sr = moosefs_sr
    ...
    @pytest.mark.reboot
    @pytest.mark.small_vm
    def test_reboot(self, vm_on_moosefs_sr: VM, host: Host, moosefs_sr: SR):
        sr = moosefs_sr
        vm = vm_on_moosefs_sr
