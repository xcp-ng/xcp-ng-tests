...
def host_with_zfsvol(host_with_saved_yum_state_toolstack_restart: Host) -> Generator[Host]:
...
@pytest.fixture(scope='package')
def zfsvol_sr(host: Host, sr_disk_wiped: str, host_with_zfsvol: Host) -> Generator[SR]:
...
def vdi_on_zfsvol_sr(zfsvol_sr: SR) -> Generator[VDI]:
    vdi = zfsvol_sr.create_vdi('ZFS-local-VDI-test')
    yield vdi
    vdi.destroy()

@pytest.fixture(scope='module')
def vm_on_zfsvol_sr(host: Host, zfsvol_sr: SR, vm_ref: str) -> Generator[VM]:
