...
# This file contains 2 tests to check that the hvmloader PCI bar fix is set or not
# by xenopsd. Tests check the xenopsd configuration and then compare with what is
# seen inside the VM. If the grant-tables are cached the fix is applied.
#
# Requirements:
# - At least one XCP-ng host (>=8.3)
# - A Linux VM

def xen_platform_pci_bar_uc(host):
    ...
    # No param means 'xen-platform-pci-bar-uc' is true by default.
    return m is None or m.group(1) == "true"

def xen_platform_pci_id(running_unix_vm):
    pci_path = running_unix_vm.ssh(['ls', '-d', '/sys/bus/pci/drivers/xen-platform-pci/0000:*'])
    if pci_path is not None:
        return os.path.basename(pci_path)
    pytest.fail("'xen-platform-pci' PCI not found")

def xen_platform_pci_io_address(unix_vm, xen_platform_pci_id):
    # Find 'xen-platform-pci' IO mem resource address.
    pci_resource_output = unix_vm.ssh(
...
def mtrr_ranges(unix_vm):
    # List and check MTRR ranges.
    return unix_vm.ssh(['cat', '/proc/mtrr']).splitlines()
...
class TestXenPlatformPciBarUc:
    ...
    def test_xen_platform_pci_bar_uc_true(self, host, running_unix_vm):

        if not xen_platform_pci_bar_uc(host):
            pytest.fail("'xen-platform-pci-bar-uc' found in '/etc/xenopsd.conf.d/' with value 'false'.")

        pci_id = xen_platform_pci_id(running_unix_vm)

        pci_io_address = xen_platform_pci_io_address(running_unix_vm, pci_id)

        logging.info(f"'xen-platform-pci' PCI IO mem address is 0x{pci_io_address:08X}")

        mtrr_ranges_list = mtrr_ranges(running_unix_vm)
    ...
    def test_xen_platform_pci_bar_uc_false(self, host_with_xen_platform_pci_bar_uc_set_to_false,
        ...
        if xen_platform_pci_bar_uc(host_with_xen_platform_pci_bar_uc_set_to_false):
            pytest.fail("'xen-platform-pci-bar-uc' not found in '/etc/xenopsd.conf.d/' or set to 'true'.")
