...
def skip_package_source(version: str, package_source: str) -> tuple[bool, str]:
    ...
    if package_source == "iso":
        if config.install.isos.definitions[version].net_only or False:
    ...
    if package_source == "net":
        ...
        if config.install.isos.definitions[version].net_url is None:
...
def installer_iso(request: pytest.FixtureRequest) -> dict[str, str | bool]:
    ...
    return dict(iso=local_iso,
                unsigned=config.install.isos.definitions[iso_key].unsigned or False,
                )
