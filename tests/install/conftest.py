...
def vm_booted_with_installer(
    ...
    # Start and pause the host VM with the ISO mounted
    host_vm.insert_cd(maybe_remastered_iso)
    host_vm.start(paused=True)
    assert host_vm.is_paused()
    ...
    installer_vm = InstallerVM.connect(host_vm)
    defer(installer_vm.cleanup)

    # Unpause the VM and wait for the running state
    host_vm.unpause()
    assert host_vm.is_running()
