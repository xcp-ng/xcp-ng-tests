...
class TestGuestToolsUnix:
    ...
    def test_install(self, running_vm, state):
        ...
        if state.vm_distro == "alpine":
            pytest.skip(
                "Alpine not supported by the guest tools installation script at the moment"
            )

        # Check that we are able to detect that xe-daemon is running
        assert (
            vm.ssh_with_result(["pgrep", "-x", "xe-daemon"]).returncode == 0
        ...
        wait_for(
            lambda: vm.ssh_with_result(["pgrep", "-x", "xe-daemon"]).returncode == 0,
    ...
    def test_xenstore(self, running_vm):
        ...
        assert (
            vm.ssh_with_result(["xenstore-exists", "data/test-xcp-ng"]).returncode != 0
        )
    ...
    def test_storage_migration(
        self, running_vm, host, hostA2, local_sr_on_hostA2, state
    ):
        vm = running_vm
        # migrate to default SR on hostA2
        vm.migrate(hostA2, local_sr_on_hostA2)
        wait_for(
            lambda: vm.all_vdis_on_host(hostA2), "Wait for all VDIs on destination host"
        )
        wait_for(
            lambda: vm.is_running_on_host(hostA2),
