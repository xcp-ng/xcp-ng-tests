...
def pif_ipv4_state(pif: PIF, state: XeParams | None) -> XeParams:
    ...
    if state is not None and state != previous:
        logging.info(
            f"Reconfiguring to '{previous['mode']}->{state['mode']}': "
            f"PIF {pif.uuid} (on '{pif.param_get('network-name-label') or '?'}')",
        )
        state['uuid'] = pif.uuid
...
class Dhclient(DHCPClient):
    ...
    def is_running(self) -> bool:
        ...
        for lease in leases:
            ...
            routers_value = lease.get('option:routers')
            routers = [] if routers_value is None else routers_value.split(',')
