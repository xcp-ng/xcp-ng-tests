...
def tcpdump(
    ...
    *,
    ...
    # cleanup() is also called with defer(), but it is called here because tcpdump() is also a contextmanager.
    # so we need to cleanup resources once the contextmanager is terminated.
    # cleanup() could be called several times without issues, so it is safe to do both.
    cleanup()
