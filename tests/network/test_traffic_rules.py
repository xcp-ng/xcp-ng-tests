...
class TestSimple:
    def test_vifRule(self, hosts_with_xo: list[Host], imported_vm: VM):
    ...
    def test_networkRule(self, hosts_with_xo: list[Host], imported_vm: VM):
        host = hosts_with_xo[0]
        vm = imported_vm.clone()
...
@pytest.mark.small_vm
class TestMigrate:
    def test_vifRule(self, hosts_with_xo: list[Host], hostA2: Host, local_sr_on_hostA2, running_vm: VM):
        hostA1 = hosts_with_xo[0]
    ...
    def test_networkRule(self, hosts_with_xo: list[Host], hostA2: Host,
                         local_sr_on_hostA1, local_sr_on_hostA2, running_vm: VM):
        hostA1 = hosts_with_xo[0]
        ...
        # the test is assymetric, start with the VM on know host
        if not vm.is_running_on_host(hostA1):
            vm.migrate(hostA1, local_sr_on_hostA1)
        ...
        try:
            ...
            # the rule is applied on hostA1 only (icmp to 10.0.0.1 is blocked)
            ...
            # the rule is applied on hostA2 only (icmp to 10.0.0.1 is blocked)
...
@pytest.mark.small_vm
class TestVLAN:
    def test_vifRule(self, hosts_with_xo: list[Host], imported_vm: VM, empty_network: Network,
                     vlan: VLAN):
        host = hosts_with_xo[0]
    ...
    def test_networkRule(self, hosts_with_xo: list[Host], imported_vm: VM,
                         empty_network: Network, vlan: VLAN):
        host = hosts_with_xo[0]
        network = empty_network
        vm = imported_vm.clone()
        vlan_tag = vlan.tag()
...
class TestTunnel:
    def test_vifRule(self, hosts_with_xo: list[Host], imported_vm: VM,
                     tunnel: Tunnel, tunnel_protocol: str):
        host = hosts_with_xo[0]
