...
from lib.packagemanager import AptGetPackageManager, DnfPackageManager, Package, YumPackageManager
...
class TestGuestToolsUnix:
    ...
    def vm_install(self, running_vm: VM, state: State) -> None:
        ...
        if isinstance(pkg_mgr, YumPackageManager | DnfPackageManager):
        ...
        # the device may not be immediately available. Retry to mount until it succeeds
        wait_for(lambda: vm.ssh_with_result(f'mount -t iso9660 /dev/cdrom {tmp_mnt}').returncode == 0)
