...
def pytest_sessionstart(session: pytest.Session) -> None:
    # Placed here rather than in pytest_configure: the colored live-log handler is only
    # attached from pytest_sessionstart on, so warnings logged in pytest_configure are
    # printed raw and uncolored by the root logger.
    warn_legacy_data_py()
