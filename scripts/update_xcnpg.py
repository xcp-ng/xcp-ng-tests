...
def main():
    ...
    # How do we know where the VM is? Pass the host as a parameter?
    #     So add two parameters, one of which is optional for the VMs?
    # We need to know if the VM and if the VM will take a snapshot.
    # But we need to know the VM's UUID and the host's IP for it to take the snapshot.
