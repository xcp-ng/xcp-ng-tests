...
Environment variables:
  ...
  HOST_DEFAULT_PASSWORD   Default password for XAPI authentication
    ...
    #  First positional argument can be either URL in XOA mode, or output_file in
    #  XOLite mode.
    parser.add_argument('url_or_filename', nargs='?', help=argparse.SUPPRESS)
    #  Second positional argument only used in XOLite mode
    parser.add_argument('filename', nargs='?', help=argparse.SUPPRESS)

    # XOLite mode options
    parser.add_argument('--host', help="Host IP for XOLite mode")
    parser.add_argument('--vm-uuid', help="VM UUID for XOLite mode")
    parser.add_argument('--user', help="Username (default: HOST_DEFAULT_USER env var or 'root')")
    parser.add_argument('--password', help="Password (default: HOST_DEFAULT_PASSWORD env var)")

    # XOA mode options
    parser.add_argument('--cookie', help="Cookie for XOA authentication")

    # Common options
    parser.add_argument('--output-dir', dest='output_dir', help="Output directory for screenshot")
