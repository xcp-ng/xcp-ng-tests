...
### Using lib/config.toml

Test configuration is managed via TOML files. The project includes a `lib/config.toml` file with all default settings.

#### Default configuration

The `lib/config.toml` file contains all default settings for:
...
2. **Run tests with custom config**:
   ...
   This loads `lib/config.toml` first, then merges `config.local.toml` on top.

#### Configuration file paths

- `lib/config.toml` — base config (always loaded, lowest priority)
...
1. `lib/config.toml` is loaded first
...
loads `lib/config.toml` first, then merges `../../inventory/cgt1-qcow2.toml` on top.
...
This allows you to:
- Commit `lib/config.toml` with project defaults to version control
...
* `OVA_URL` is a URL to download an OVA. It can also be simply a filename, if your `lib/config.toml`'s `vm.def_url` is correctly defined.
* `VM_key` refers to a key in `lib/config.toml`'s `vm.images` section. Example: `mini-linux-x86_64-uefi`.
...
The script requires the addressable name or IP of the PXE config server to be defined in `lib/config.toml`:
```toml
[pxe]
config_server = "pxe"
```
