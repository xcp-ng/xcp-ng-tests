...
def customize_isolinux(installer_vm: InstallerVM, vmlinuz_config: str) -> None:
    ...
    for attempt in range(30):
        installer_vm.send_to_console(sentinel)
        time.sleep(1)
        installer_vm.refresh_screen()
        if installer_vm.has_content_on_screen(sentinel):
            break
    else:
        raise installer_vm.screen_error("No sentinel on screen")
    installer_vm.debug_screen("Echo screen")

    # Force the boot prompt to appear
    installer_vm.send_to_console("sentinel2\rmenu")
