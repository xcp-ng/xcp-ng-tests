...
def load_config(
    ...
    The main lib/config.toml is always used as the base (lowest
    ...
    base_path = config_path or REPO_ROOT / "lib" / "config.toml"
    try:
...
def apply_override(config_name: str | None = None, config_values: list[str] | None = None) -> None:
    """Load lib/config.toml, merge the overlay (a .toml file path or profile name) on top, update config in place.
