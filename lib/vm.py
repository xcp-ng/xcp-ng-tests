...
class VM(BaseVM):
    ...
    def insert_cd(self, vdi_name, use_cache=False):
        ...
        # FIXME: rescan only the right ISO (if at all possible)
        # FIXME: rescan after copying rather than on each use (this
        # function can be used also for standard ISOs not generated by our tests)
        if not use_cache:
