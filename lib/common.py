...
def wait_for(fn: Callable[[], Any], msg: str | None = None, timeout_secs: int = 2 * 60, retry_delay_secs: int = 2,
...
def wait_for_not(*args, **kwargs) -> None:
    kwargs['invert'] = True
    return wait_for(*args, **kwargs)
...
def strtobool(str_val: Optional[str]) -> bool:
    ...
    if str_val in ('y', 'yes', 't', 'true', 'on', '1'):
        return True
    if str_val in ('n', 'no', 'f', 'false', 'off', '0'):
        return False
    raise ValueError("invalid truth value '{}'".format(str_val))
...
def _param_set(host: 'Host', xe_prefix: str, uuid: str, param_name: str, value: str, key: Optional[str] = None) -> None:
...
def _param_add(host: 'Host', xe_prefix: str, uuid: str, param_name: str, value: str, key: Optional[str] = None) -> None:
