...
def callable_marker(value, request, param_mapping):
    ...
    Such markers take 2 parameters:
    - one or more value(s), or callable(s) that will return a value
    - a mapping of parameters to test fixture names, to pass as named arguments
      to the callable
    """
    if callable(value):
        try:
            params = { arg_name: request.getfixturevalue(fixture_name)
                       for arg_name, fixture_name in param_mapping.items() }
