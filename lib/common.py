...
def wait_for(fn, msg=None, timeout_secs=120, retry_delay_secs=2, invert=False):
    ...
    while True:
        ...
        if time_left <= 0:
            ...
            raise Exception(
                "Timeout reached while waiting for fn call to yield %s (%s)."
...
class Pool:
    ...
    def host_ip(self, host_uuid):
        return self.master.xe(
            "host-param-get", {"uuid": host_uuid, "param-name": "address"}
        )
...
class Host:
    ...
    def is_enabled(self):
        try:
            return (
                self.xe("host-param-get", {"uuid": self.uuid, "param-name": "enabled"})
                == "true"
            )
    ...
    def yum_install(self, packages, enablerepo=None, save_state=False):
        ...
        enablerepo_cmd = (
            ["--enablerepo=%s" % enablerepo] if enablerepo is not None else []
        )
    ...
    def yum_restore_saved_state(self):
        ...
        self.ssh(
            [
                ...
                "history",
                "rollback",
                "--enablerepo=xcp-ng-base,xcp-ng-testing,xcp-ng-updates",
                self.saved_rollback_id,
                "-y",
            ]
        )
        pkgs = self.packages()
        if self.saved_packages_list != pkgs:
            ...
            raise Exception(
                "Yum state badly restored missing: [%s], extra: [%s]."
    ...
    def management_network(self):
        return self.xe(
            "network-list",
            {"bridge": self.inventory["MANAGEMENT_INTERFACE"]},
            minimal=True,
        )
    ...
    def local_vm_srs(self):
        srs = []
        for sr_uuid in self.xe(
            "pbd-list", {"host-uuid": self.uuid, "params": "sr-uuid"}, minimal=True
        ).split(","):
            sr = SR(sr_uuid, self.pool)
...
class BaseVM:
    ...
    def param_get(self, param_name, key=None, accept_unknown_key=False):
        ...
        try:
            ...
            if (
                key
                and accept_unknown_key
                and e.stdout.decode().strip() == "Error: Key %s not found in map" % key
            ):
                value = None
            else:
                raise
    ...
    def get_vdi_sr_uuid(self, vdi_uuid):
        return self.host.xe(
            "vdi-param-get", {"uuid": vdi_uuid, "param-name": "sr-uuid"}
        )
    ...
    def export(self, filepath, compress="false"):
        ...
        params = {"uuid": self.uuid, "compress": compress, "filename": filepath}
...
class VM(BaseVM):
    def __init__(self, uuid, host):
        ...
        self.is_windows = (
            self.param_get("platform", "device_id", accept_unknown_key=True) == "0002"
        )
    ...
    def shutdown(self, force=False, verify=False):
        print("Shutdown VM")
        ret = self.host.xe(
            "vm-shutdown", {"uuid": self.uuid, "force": to_xapi_bool(force)}
        )
        if verify:
    ...
    def reboot(self, force=False, verify=False):
        print("Reboot VM")
        ret = self.host.xe(
            "vm-reboot", {"uuid": self.uuid, "force": to_xapi_bool(force)}
        )
        if verify:
            # a way to test that the reboot is happening is that SSH doesn't respond anymore
            wait_for_not(self.is_ssh_up, "Wait for SSH down")
            # and then we wait for the VM to be up again, including SSH
    ...
    def is_management_agent_up(self):
        return (
            self.param_get("PV-drivers-version", "major", accept_unknown_key=True)
            is not None
        )
    ...
    def migrate(self, target_host, sr=None):
        ...
        wait_for(
            ...
            "Wait for XO to know about VM %s" % self.uuid,
        )

        msg = "Migrate VM to host %s" % target_host
        params = {"vm": self.uuid, "targetHost": target_host.uuid}
        if sr is not None:
            msg += " (SR: %s)" % sr.uuid
            params["sr"] = sr.uuid
        print(msg)
    ...
    def snapshot(self):
        ...
        return Snapshot(
            self.host.xe(
                "vm-snapshot",
                {"uuid": self.uuid, "new-name-label": '"Snapshot of %s"' % self.uuid},
            ),
            self.host,
        )
    ...
    def checkpoint(self):
        ...
        return Snapshot(
            self.host.xe(
                "vm-checkpoint",
                {"uuid": self.uuid, "new-name-label": '"Checkpoint of %s"' % self.uuid},
            ),
            self.host,
        )
    ...
    def vifs(self):
        _vifs = []
        for vif_uuid in self.host.xe(
            "vif-list", {"vm-uuid": self.uuid}, minimal=True
        ).split(","):
    ...
    def start_background_process(self, cmd):
        ...
        with tempfile.NamedTemporaryFile("w") as f:
            ...
            wait_for(
                lambda: self.ssh_with_result(["test", "-f", pidfile]),
...
class SR:
    ...
    def pbd_uuids(self):
        return self.pool.master.xe(
            "pbd-list", {"sr-uuid": self.uuid}, minimal=True
        ).split(",")
    ...
    def all_pbds_attached(self):
        ...
        for pbd_uuid in self.pbd_uuids():
            all_attached = (
                all_attached
                and self.pool.master.xe(
                    "pbd-param-get",
                    {"uuid": pbd_uuid, "param-name": "currently-attached"},
                )
                == "true"
            )
        return all_attached
    ...
    def exists(self):
        return (
            self.pool.master.xe("sr-list", {"uuid": self.uuid}, minimal=True)
            == self.uuid
        )
    ...
    def hosts_uuids(self):
        return self.pool.master.xe(
            "pbd-list", {"sr-uuid": self.uuid, "params": "host-uuid"}, minimal=True
        ).split(",")
    ...
    def content_type(self):
        return self.pool.master.xe(
            "sr-param-get", {"uuid": self.uuid, "param-name": "content-type"}
        )

    def is_shared(self):
        return (
            self.pool.master.xe(
                "sr-param-get", {"uuid": self.uuid, "param-name": "shared"}
            )
            == "true"
        )
...
def cold_migration_then_come_back(vm, prov_host, prov_sr, dest_host, dest_sr):
    ...
    wait_for(
        lambda: vm.all_vdis_on_host(dest_host), "Wait for all VDIs on destination host"
    )
    ...
    wait_for(
        lambda: vm.all_vdis_on_host(prov_host),
        "Wait for all VDIs back on provenance host",
    )
    # Start VM to make sure it works
    vm.start()
    vm.wait_for_os_booted()
...
def live_storage_migration_then_come_back(vm, prov_host, prov_sr, dest_host, dest_sr):
    ...
    wait_for(
        lambda: vm.is_running_on_host(prov_host),
        "Wait for VM to be running on provenance host",
    )
    vm.shutdown(verify=True)
