...
from lib.config_loader import InstalledGuestToolDef
from lib.vm import VM

from . import WINDOWS_SHUTDOWN_COMMAND, enable_testsign, insert_cd_safe, wait_for_vm_running_and_ssh_up_without_tools

def install_other_drivers(vm: VM, other_tools_iso_name: str, param: InstalledGuestToolDef) -> None:
    ...
    driver_type = param.type
    if driver_type is not None:
        assert param.path is not None and param.package is not None
        insert_cd_safe(vm, other_tools_iso_name)

        if param.testsign_cert:
