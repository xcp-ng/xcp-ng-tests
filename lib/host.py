...
class Host:
    ...
    def iso_sr_uuid(self):
        hostname = self.xe("host-param-get", {"uuid": self.uuid, "param-name": "name-label"})
        local_sr_uuids = safe_split(
            self.xe(
                "sr-list",
                {"host": hostname, "content-type": "iso", "type": "iso", "shared": "false", "minimal": "true"},
            ),
            ",",
        )
        return local_sr_uuids[0]

    def import_iso(self, uri, sr_uuid):
        ...
        return VDI(vdi_uuid, sr=SR(sr_uuid, self.pool))
